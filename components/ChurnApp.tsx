  ...
  return (
    <svg width={124} height={124} viewBox="0 0 124 124">
      ...
      <circle cx={62} cy={62} r={r} fill="none" stroke={color} strokeWidth={9}
        strokeDasharray={`${dash} ${circ}`} strokeLinecap="round" transform="rotate(-90 62 62)" />
      <text x={62} y={58} textAnchor="middle" fill={color} fontSize={22} fontWeight={800}>{pct}%</text>
      <text x={62} y={74} textAnchor="middle" fill="var(--muted)" fontSize={9} fontWeight={600} letterSpacing="0.6">
        CHURN PROB
      </text>
    </svg>
  )
...
function RecommendationsList({ items, loading }: { items: string[]; loading: boolean }) {
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {[1, 2, 3, 4].map(i => (
          <div key={i} style={{ height: 14, borderRadius: 6, background: 'var(--border2)',
            width: i === 4 ? '60%' : '100%', opacity: 0.6, animation: 'pulse 1.5s infinite' }} />
        ))}
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {items.map((rec, i) => (
        <div key={i} style={{ display: 'flex', gap: 10, fontSize: '0.84rem', color: 'var(--text)', lineHeight: 1.55 }}>
          <span style={{ color: 'var(--accent2)', flexShrink: 0, fontWeight: 700, marginTop: 1 }}>›</span>
          <span>{rec}</span>
        </div>
      ))}
    </div>
...
export default function ChurnApp() {
  ...
  const [batchRecs, setBatchRecs]         = useState<string[]>([])
  const [batchLoading, setBatchLoading]   = useState(false)
  const [batchRecsLoading, setBatchRecsLoading] = useState(false)
  ...
  function setF<K extends keyof FormInput>(k: K, v: number) {
    setForm(p => ({ ...p, [k]: v }))
    setResult(null); setRecs([])
  }

  async function fetchRecommendations(customer: FormInput, predResult: PredResult) {
    setRecsLoading(true)
    try {
      const res = await fetch('/api/recommend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ customer, result: predResult, mode: 'single' }),
      })
  ...
  async function handlePredict() {
    ...
    try {
      ...
      setResult(data)
      fetchRecommendations(form, data)
    } catch (e) {
      setError(String(e))
    } finally {
      setLoading(false)
  ...
  async function fetchBatchRecommendations(br: BatchResult) {
    setBatchRecsLoading(true)
    try {
      const res = await fetch('/api/recommend', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ result: br, mode: 'batch' }),
      })
      const data = await res.json()
      setBatchRecs(data.recommendations ?? [])
    } catch { setBatchRecs([]) } finally { setBatchRecsLoading(false) }
  }
  ...
  async function handleBatch(file: File) {
    ...
    try {
      ...
      const customers = parsed.data.map(row => {
        ...
        for (const [k, v] of Object.entries(row)) out[k.trim().toLowerCase()] = parseFloat(v) || 0
        return out
      })
      ...
      if (data.error) throw new Error(data.error)
      setBatchResult(data)
      fetchBatchRecommendations(data)
    } catch (e) {
  ...
  function onFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    const f = e.target.files?.[0]; if (f) handleBatch(f)
  }
  function onDrop(e: React.DragEvent) {
    e.preventDefault(); setDragOver(false)
    const f = e.dataTransfer.files?.[0]; if (f) handleBatch(f)
  }
      ...
      <div style={{ maxWidth: 1060, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', right: 40, top: -10, fontSize: 180,
            opacity: 0.04, lineHeight: 1, userSelect: 'none' }}>⟳</div>
          <p className="section-label">Machine Learning · AI-Powered</p>
          ...
          <p style={{ color: '#fcd27a', fontSize: '1rem', maxWidth: 560 }}>
            XGBoost churn prediction with Gemini-powered retention recommendations. Enter customer metrics for real-time risk assessment and actionable insights.
          </p>
          <div style={{ marginTop: 20, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['XGBoost Model', '6 Features', 'Gemini AI', 'Real-time'].map(t => (
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 28 }}>
          {(['single', 'batch'] as const).map(t => (
            <button key={t} className={`tab${tab === t ? ' active' : ''}`} onClick={() => setTab(t)}>
              {t === 'single' ? 'Single Prediction' : 'Batch Upload'}
            </button>
          ))}
        </div>
        ...
        {tab === 'single' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
              ...
              <div className="card">
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button onClick={() => { setForm(DEFAULT_FORM); setResult(null); setRecs([]); setError('') }}
            ...
            {/* AI Recommendations — full width below */}
            {(result || recsLoading) && (
              <div className="card">
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
                  <p className="section-label" style={{ margin: 0 }}>Gemini AI</p>
                  <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
                  {recsLoading && (
                    <span style={{ fontSize: '0.76rem', color: 'var(--muted)' }}>Generating…</span>
                  )}
                </div>
                <p className="section-title" style={{ marginBottom: 16 }}>Retention Recommendations</p>
                <RecommendationsList items={recs} loading={recsLoading} />
              </div>
            )}
          </div>
        )}
            ...
            <div className="card">
              ...
              >
                ...
                <div style={{ color: 'var(--muted)', fontSize: '0.8rem' }}>
                  Columns: tenure, numberofaddress, cashbackamount, daysincelastorder, ordercount, satisfactionscore
                </div>
                <input ref={fileRef} type="file" accept=".csv" hidden onChange={onFileChange} />
              </div>
              {batchLoading && <div style={{ color: 'var(--muted)', fontSize: '0.86rem' }}>Processing {fileName}…</div>}
              {batchError && (
                <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)',
            ...
            {batchResult && !batchResult.error && (
              <>
                ...
                <div className="card">
                  ...
                  <p className="section-title" style={{ marginBottom: 16 }}>Strategic Retention Recommendations</p>
                  <RecommendationsList items={batchRecs} loading={batchRecsLoading} />
                </div>
                ...
                <div className="card">
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(190px,1fr))', gap: 10 }}>
                    {batchResult.predictions.map(row => {
                      const bc = row.risk_level === 'high' ? 'rgba(239,68,68,0.25)'
                        : row.risk_level === 'medium' ? 'rgba(245,158,11,0.25)' : 'rgba(23,192,130,0.25)'
                      return (
                        <div key={row.row_id} style={{ background: 'var(--surface2)', borderRadius: 8,
                          border: `1px solid ${bc}`, padding: '10px 14px' }}>
